...
# movesStr2 = "U7,R6,D4,L4"
movesStr2 = "U62,R66,U55,R34,D71,R55,D58,R83"
# movesStr2 = "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7"
pathTwoList = movesStr2.split(",")
...
def getCoordinates(list):
    ...
    for command in list:
        ...
        commandNum = int(command[1:])
        
        
        if command[0] == 'R':
            newCoordX = coordX + commandNum
            for num in range(coordX, newCoordX+1):
                if coordinatesDict[num]: 
                    coordinatesDict.setdefault(key, []).append(num)
                else:
                    coordinatesDict[num] = coordY
            coordX += commandNum
        if command[0] == 'L':
            newCoordX = coordX - commandNum
            for num in range(coordX, newCoordX - 1, -1):
                if coordinatesDict[num]: 
                    coordinatesDict.setdefault(key, []).append(num)
                else:
                    coordinatesDict[num] = coordY
            ...
            newCoordY = coordY + commandNum
            for num in range(coordY, newCoordY + 1):
                coordinatesDict[coordX] = num
            coordY += commandNum
        elif command[0] == 'D':
            newCoordY = coordY - commandNum
            for num in range(coordY, newCoordY - 1, -1):
                coordinatesDict[coordX] = num
            coordY -= commandNum
...
# testList = ['R5','L2', 'U5', 'D2']
# print(getCoordinates(testList))

pathOneCoordinates = getCoordinates(pathOneList)
print(pathOneCoordinates)


pathTwoCoordinates = getCoordinates(pathTwoList)
# print(pathTwoCoordinates)


for item in pathOneCoordinates:
    for num in pathTwoCoordinates:
        if item == num:
            print(item)
...
#                 dictItem = {}
#                 dictItem[num] = coordY
#                 coordinatesList.append(dictItem)
#             coordX += commandNum
#         if command[0] == 'L':
#             newCoordX = coordX - commandNum
#             for num in range(coordX, newCoordX - 1, -1):
...
# def rangeBetweenCoordinates(list):
#     coordinateRangeSet = []
...
#         # print((list[i][0][0]), (list[i+1][0][0]))
#         for num in range((list[i][0][0]), (list[i+1][0][0])):
#             if num == 0:
#                 xRange.append()
#             xRange.append(num)
#         for num in range((list[i][1][0]), (list[i+1][1][0])):
#             yRange.append(num)

#         coordinateRangeSet.append([xRange, yRange])
#     return coordinateRangeSet
          
        
# print(rangeBetweenCoordinates(pathOneCoordinates))
...
#             xList.append(coordX)
#             yList.append(coordY)
#         coordinatesList.append([xList, yList])
#     return coordinatesList
...
#             newCoordX = coordX + commandNum
#             coordXList = []
#             for x in range(coordX, newCoordX):
#                 coordXList.append(x)
#             print(coordXList)
#             coordinateSet.append(coordXList)
#             coordinateSet.append([coordY])
#             coordX += commandNum
#         elif command[0] == 'L':
#             newCoordX = coordX - commandNum
#             coordXList = []
...
#             newCoordY = coordY + commandNum
#             coordYList = []
#             for y in range(coordY, newCoordY):
#                 coordYList.append(y)
#             coordinateSet.append([coordX])
#             coordinateSet.append(coordYList)
#             coordY += commandNum
#         elif command[0] == 'D':
#             newCoordY = coordY - commandNum
#             coordYList = []
...
#             coordinateSet.append(coordYList)
#             coordY -= commandNum
#         coordinateSet.append(coordinatesList)
#     return coordinatesList  
