...
def plotRight(command):
    commandNum = int(command[1:])
    print(command)
    for i in range(1, int(command[1:])+1):
        if board[centralPortY][centralPortX + i] in ["+", "-", "|","O"]:
            board[centralPortY][centralPortX + i] = "X"
            print(f"Intersection X-Coord: {centralPortX},Y-Coord:{centralPortY + i}")
        else:
            if i == commandNum:
                board[centralPortY][centralPortX + i] = "+"
            else:
                board[centralPortY][centralPortX + i] = "-"
            
def plotLeft(command):
    commandNum = int(command[1:])
    print(command)
    for i in range(1, int(command[1:])+1):
        if board[centralPortY][centralPortX - i] in ["+", "-", "|","O"]:
...
def plotUp(command):
    ...
    print(command)
    for i in range(1, int(command[1:])+1):
        if board[centralPortY - i][centralPortX] in ["+", "-", "|","O"]:
            board[centralPortY - i][centralPortX] = "X"
            print(f"Intersection X-Coord: {centralPortX - i},Y-Coord:{centralPortY}")
        else:
            if i == commandNum:
                board[centralPortY - i][centralPortX] = "+"
            else:
                board[centralPortY - i][centralPortX] = "|"

def plotDown(command):
    commandNum = int(command[1:])
    print(command)
    for i in range(1, int(command[1:])+1):
        if board[centralPortY + i][centralPortX] in ["+", "-", "|","O"]:
...
def plotPoint(location, command, size=20):
...
while status:
    ...
    centralPortX = int(size/2)-1
    centralPortY = int(size/2)-1
    board[centralPortY][centralPortX] = "O"
    movesList = ["R2", "U5", "L4"]
    counter = 0
